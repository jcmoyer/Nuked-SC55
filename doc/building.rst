...
``-DCMAKE_PREFIX_PATH=<path>`` where ``<path>`` points to a directory
containing SDL2, and optionally rtmidi (only when ``-DUSE_RTMIDI=ON``).
...
SDK obtained from `here <asio-sdk_>`_.
...
- ``-DNUKED_TEST_JV880_NVRAM=<path>``: ``<path>`` should point to a file
  ...
  appended to it. This is the emulator instance number and should **not** be
  included in the filename passed to cmake.
